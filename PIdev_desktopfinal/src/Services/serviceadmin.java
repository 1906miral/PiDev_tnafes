...
public class serviceadmin implements IServiceUser<admin> {
    ...
    admin rechercheamin(int id) {
        ...
        try {
            ...
            while (rs.next()) {
                ad = new admin(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4),rs.getInt(5),rs.getString(6));
                return ad;
            }
    ...
    public void ajouter(admin t) {
        try {
            ...
            for (user u : listu){
                requete = "INSERT INTO adminstrateur (id_admin,nom,prenom,email,numtel) VALUES ("+u.getId()+",'"+t.getNom()+"','"+t.getPrenom()+"','"+t.getEmail()+"',"+t.getNumtel()+")";
