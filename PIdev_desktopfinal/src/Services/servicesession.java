...
public class servicesession {
    ...
    public List<admin> rechercheadmin(String email) {
        ...
        try {
            String requete = "SELECT * FROM adminstrateur WHERE email='"+email+"'";
    ...
    public List<coach> recherchecoach(String email) {
        ...
        try {
            String requete = "SELECT * FROM coach WHERE email='"+email+"'";
            Statement st = cnx.createStatement();
            ResultSet rs = st.executeQuery(requete);
            while (rs.next()) {
    ...
    public boolean bancheck(String email) {
        try {
            ...
            ResultSet rs = st.executeQuery(requete);
            rs.next();
            return rs.getBoolean("perma_ban");
        } catch (SQLException ex) {
            System.err.println(ex.getMessage());
        }
        return false;
